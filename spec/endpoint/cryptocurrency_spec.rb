...
RSpec.describe CoinMarketPro::Endpoint::Cryptocurrency do
  ...
  let(:uri) { '/cryptocurrency' }

  describe '#info' do
    let(:endpoint) { uri + '/info' }

    before(:each) do
      stub_request(:get, /#{endpoint}/).and_return(body: fixture('cryptocurrency/info.json'))
    end
  ...
  describe '#market_pairs' do
    ...
    it 'raises error when invalid params' do
      expect { subject.market_pairs(foo: 1) }.to raise_error(ArgumentError, 'At least one "id" or "symbol" is required.')
      expect(a_request(:get, /#{endpoint}/)).not_to have_been_made
    end
  end

  describe '#ohlcv_historical' do
    let(:endpoint) { uri + '/ohlcv/historical' }

    before(:each) do
      stub_request(:get, /#{endpoint}/).and_return(body: fixture('cryptocurrency/ohlcv_historical.json'))
    end

    it 'returns successful results' do
      result = subject.ohlcv_historical(id: [1])
    ...
    it 'raises error when invalid params' do
      expect { subject.ohlcv_historical(foo: 1) }
        .to raise_error(ArgumentError, 'At least one "id" or "symbol" is required.')
      expect(a_request(:get, /#{endpoint}/)).not_to have_been_made
    end
  end
  ...
  describe '#ohlcv' do
    let(:endpoint) { uri + '/ohlcv/latest' }

    before(:each) do
      stub_request(:get, /#{endpoint}/).and_return(body: fixture('cryptocurrency/ohlcv.json'))
    end

    it 'returns successful results' do
      result = subject.ohlcv(id: [1])
    ...
    it 'alias ohlcv_latest returns successful results' do
      result = subject.ohlcv_latest(id: [1])
